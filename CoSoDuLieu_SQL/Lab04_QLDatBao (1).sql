...
Else-- sau khi kiểm tra trùng kháo chính tiến hành thêm dữ liệu.
	begin
...
from KhachHang A, DatBao B
where A.MaKH = B.MaKH and MaBaoTC not in(select  D.MaBaoTC
                                          from KhachHang C, DatBao D
                                          where  A.MaKH = B.MaKH and MaBaoTC like 'TN%' )
--6)	Cho biết số tờ báo mà mỗi khách hàng đã đặt mua.
select  A.TenKH, B.MaBaoTC, C.SoBaoTC
from KhachHang A, DatBao B, PhatHanh C
...
select B.TenKH, A.Ten, DinhKy, SLMua, SLMua * GiaBan as sotien 
...
select A.MaBaoTC
from PhatHanh A, DatBao B
where A.MaBaoTC = B.MaBaoTC and B.MaBaoTC not in
(select C.MaBaoTC
from DatBao C
)
--12)	Cho biết tên, định kỳ của những tờ báo có nhiều người đặt mua nhất.
select Ten, DinhKy, B.SLMua
from Bao_TChi A , DatBao B
where A.MaBaoTC = B.MaBaoTC and SLMua >= all(
               select D.SLMua
